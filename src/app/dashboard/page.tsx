  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-600 mt-1">Acesse pacientes, vínculos assistenciais, teleconsultas, documentos, IA, cobranças e dados autorizados</p>
      </div>

      <div className="grid md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-4 mb-8">
        <Link href="/meus-pacientes" className="bg-gradient-to-br from-emerald-700 to-slate-950 text-white rounded-2xl p-5 shadow-lg hover:shadow-xl transition-shadow">
          <Users className="w-8 h-8 mb-3" />
          <h2 className="font-bold text-lg">Meus Pacientes</h2>
          <p className="text-sm text-white/80 mt-1">Vínculo contínuo, autorizado e revogável pelo paciente.</p>
          <span className="inline-flex items-center gap-1 mt-4 text-sm font-semibold">Acompanhar <ArrowRight className="w-4 h-4" /></span>
        </Link>
        ...
        <Link href="/financeiro" className="bg-gradient-to-br from-emerald-700 to-teal-900 text-white rounded-2xl p-5 shadow-lg hover:shadow-xl transition-shadow">
          ...
          <p className="text-sm text-white/80 mt-1">Powered by NextGen: Pix, teleconsulta, recorrência e planos.</p>
        ...
        <Link href="/teleconsultas" className="bg-gradient-to-br from-cyan-600 to-blue-700 text-white rounded-2xl p-5 shadow-lg hover:shadow-xl transition-shadow">
          ...
          <p className="text-sm text-white/80 mt-1">Agenda, chamada, cobrança, lembretes e orientações.</p>
          <span className="inline-flex items-center gap-1 mt-4 text-sm font-semibold">Abrir <ArrowRight className="w-4 h-4" /></span>
        </Link>
      ...
      <div className="bg-violet-50 border border-violet-200 rounded-2xl p-4 mb-8 flex items-start gap-3">
        ...
        <p className="text-sm text-violet-950">O MyDataMed permite acompanhamento contínuo, autorizado e inteligente — com dados, IA, agenda, documentos, CRM, teleatendimento, cobranças e automações em um só lugar.</p>
      </div>
      ...
      <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 mb-8 flex items-start gap-3">
        ...
        <p className="text-sm text-emerald-900">Novo: o Vínculo Assistencial transforma autorização temporária em carteira de pacientes acompanhados, sempre com aprovação e revogação pelo paciente.</p>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4 mb-8 flex items-start gap-3">
        ...
        <p className="text-sm text-blue-900">CNS/Cartão SUS é uma ponte operacional: ajuda profissionais, clínicas e prefeituras a organizar o cuidado, mantendo os usos de HealthWallet e MyDataMed para todos os públicos.</p>
      </div>

      <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 mb-8">
        <div className="flex items-center justify-center gap-3 mb-6"><KeyRound className="w-8 h-8 text-emerald-600" /><span className="text-lg font-semibold text-gray-900">Código de Acesso</span></div>
      ...
      <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl p-6 mb-8">
        ...
        <ul className="space-y-2 text-sm text-gray-700">
          ...
          <li className="flex items-start gap-2"><Send className="w-4 h-4 text-emerald-600 mt-0.5 flex-shrink-0" /><span>Após acessar, você pode solicitar vínculo contínuo, enviar documentos, orientações, cobranças e prescrições quando aplicável</span></li>
        </ul>
      </div>
          ...
          <div className="space-y-3">
            {loadingRecent ? <div className="text-center py-4"><Loader2 className="w-6 h-6 animate-spin text-emerald-600 mx-auto" /></div> : recentAccess.map((access) => (
              <div key={access.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                ...
                <div className="flex gap-3"><Link href={`/copiloto?patient=${access.patient_id}`} className="text-violet-700 text-sm font-medium hover:underline">Copiloto IA</Link><Link href={`/patient/${access.id}`} className="text-emerald-600 text-sm font-medium hover:underline">Ver →</Link></div>
              </div>
            ))}
          </div>
        </div>
